...
def run_pipeline():
    ...
    alerts_sent_count = 0
    new_job_ids = []  # Track NEWLY inserted jobs for per-user notifications
    ...
    try:
        ...
        print(f"Fetched {jobs_fetched_count} jobs from sources.")
        ...
        print("Scored jobs based on keyword preferences.")
        ...
        for job in jobs:
            ...
            if inserted_id:
                ...
                increment_metric("jobs_inserted")

        print(f"\nJobs saved to database: {jobs_inserted_count} new (IDs: {new_job_ids[:5]}...)")
        print(f"Total new_job_ids tracked: {len(new_job_ids)}")

        # ===== NEW PER-USER REAL-TIME NOTIFICATIONS =====
        # Industry-grade: For every new relevant job, notify matching users ASAP
        # Linked to user_notifications table (bell icon)
        if new_job_ids:
            print(f"\n--- Processing per-user notifications for {len(new_job_ids)} new jobs ---")
            notif_stats = process_new_jobs_notifications(new_job_ids)
            alerts_sent_count = notif_stats.get("telegram_sent", 0)
            print(f"Notification stats: {notif_stats}")

            # Update metrics
            for _ in range(notif_stats.get("notifications_created", 0)):
                increment_metric("alerts_sent")
        else:
            print("\nNo new jobs inserted, skipping notifications")
        ...
        print(f"\nPipeline completed in {execution_time:.2f}s — {jobs_inserted_count} new jobs, {alerts_sent_count} telegram alerts sent")

...
class UserProfile(Base):
     ...
     preferred_keywords = Column(Text)
     ...
     notification_threshold = Column(Float, default=0.6)
...
class UserNotification(Base):
    ...
    channel_status = Column(String(50), default="pending")
    created_at = Column(String(50), default=lambda: datetime.now().isoformat())
...
def fetch_unnotified_jobs():
    with get_session() as session:
        ...
        return [{"id":j.id,"title": j.title,"company": j.company,"location": j.location,"apply_link": j.apply_link,"source": j.source,"score": j.score,"is_notified": j.is_notified} for j in jobs]
...
def fetch_all_jobs_from_db(page = 1, page_size = 10,sort_by = "score", sort_order = "DESC"):
    ...
    with get_session() as session:
        ...
        jobs = session.query(Job).order_by(sort_col).offset(offset).limit(page_size).all()
        return [_job_to_dict(j) for j in jobs]

def search_jobs(keyword,limit = 20):
    search_pattern = f"%{keyword}%"
    with get_session() as session:
        jobs = session.query(Job).filter((Job.title.ilike(search_pattern)) | (Job.company.ilike(search_pattern))).order_by(Job.score.desc()).limit(limit).all()
        return [_job_to_dict(j) for j in jobs]

def filter_jobs_by_source(source, page = 1, page_size = 10):
    offset = (page - 1) * page_size
    with get_session() as session:
        jobs = session.query(Job).filter(Job.source == source).order_by(Job.score.desc()).offset(offset).limit(page_size).all()
        return [_job_to_dict(j) for j in jobs]

def _job_to_dict(job: Job) -> dict:
    return {"id": job.id,"title": job.title,"company": job.company,"location": job.location,"apply_link": job.apply_link,"source": job.source,"score": job.score,"is_notified": job.is_notified,"scraped_at": job.scraped_at,"applied_at": job.applied_at}
...
def query_jobs(keyword = None, source = None,min_score = None, page = 1, page_size = 10,sort_by = "score", sort_order = "DESC",location = None):
    ...
    with get_session() as session:
        ...
        if sort_order.upper() == "DESC":
            ...
            sort_col = sort_col.asc()
        jobs = query.order_by(sort_col).offset(offset).limit(page_size).all()
        return [_job_to_dict(j) for j in jobs]

def create_job(job_data):
    with get_session() as session:
        job = Job(title = job_data.title,company=job_data.company,location=job_data.location,apply_link=job_data.apply_link,source=job_data.source,score=job_data.score)
        session.add(job)
        session.flush()
...
def create_user(user_data):
    ...
    with get_session() as session:
        try:
            ...
            session.add(user)
            session.flush()
            result =  {"id": user.id,"username": user.username,"email": user.email,"is_verified": user.is_verified}
            session.commit()
            return result
        except IntegrityError:
            session.rollback()
...
def get_user_by_email(email : str):
    with get_session() as session:
        ...
        if not user:
            return None
        return {"id": user.id,"username": user.username,"email": user.email,"hashed_password": user.hashed_password,"is_verified": user.is_verified,"google_id": user.google_id,"auth_provider": user.auth_provider,"avatar_url": user.avatar_url}
...
def create_google_user(email: str, username: str, google_id: str, avatar_url: str = None):
    ...
    with get_session() as session:
        try:
            user = User(username = username,email = email,hashed_password = None,is_verified = True,google_id = google_id,auth_provider = "google",avatar_url = avatar_url)
            session.add(user)
            session.flush()
            result =  {"id": user.id,"username": user.username,"email": user.email,"is_verified": user.is_verified,"google_id": user.google_id,"auth_provider": user.auth_provider,"avatar_url": user.avatar_url}
            session.commit()
...
def link_google_to_existing_user(user_id: int, google_id: str, avatar_url: str = None):
    with get_session() as session:
        ...
        session.flush()
        return {"id": user.id,"username": user.username,"email": user.email,"is_verified": user.is_verified,"google_id": user.google_id,"auth_provider": user.auth_provider,"avatar_url": user.avatar_url}

def save_verification_token(user_id: int, token: str):
    with get_session() as session:
        verification_token = VerificationToken(user_id = user_id,token = token)
...
def get_verification_token(token: str):
    with get_session() as session:
        ...
        if not token_record:
            return None
        return {"id": token_record.id,"user_id": token_record.user_id,"token": token_record.token,"created_at": token_record.created_at}
...
def get_agent_overview():
    with get_session() as session:
        ...
        latest_run = session.query(PipelineRun).order_by(PipelineRun.id.desc()).first()
        successful_runs = session.query(PipelineRun).filter(PipelineRun.status == "SUCCESS").count()
        if latest_run:
            latest_data = {"run_started_at": latest_run.run_started_at,"run_completed_at": latest_run.run_completed_at,"jobs_fetched": latest_run.jobs_fetched,"jobs_inserted": latest_run.jobs_inserted,"jobs_filtered": latest_run.jobs_filtered,"duplicates_skipped": latest_run.duplicates_skipped,"scraper_failures": latest_run.scraper_failures,"alerts_sent": latest_run.alerts_sent,"status": latest_run.status,"execution_time_seconds": latest_run.execution_time_seconds}
        else:
            latest_data = {}
    return {"total_jobs": total_jobs,"successful_runs": successful_runs,"latest_run": latest_data}

def fetch_diverse_recommended_jobs(limit=3):
     with get_session() as session:
        jobs = session.query(Job).order_by(Job.score.desc()).limit(50).all()
     results = []
     seen = set()
     for j in jobs:
...
def backfill_job_embeddings(batch_size = 50):
    ...
    from app.services.vector_store import store_job_embedding
    with get_session() as session:
        jobs = session.query(Job).filter(Job.semantic_text.isnot(None),Job.semantic_text != "").all()
        rows = [{"id": j.id,"title": j.title,"company": j.company,"location": j.location,"apply_link": j.apply_link,"source": j.source,"score": j.score,"semantic_text": j.semantic_text} for j in jobs]
    total = len(jobs)
    if total == 0:
    ...
    for i in range(0, total, batch_size):
        ...
        entries = []
        for row in batch:
            entries.append({"job_id": row["id"],"metadata": {"title": row["title"] or "","company": row["company"] or "","location": row["location"] or "","apply_link": row["apply_link"] or "","source": row["source"] or "","score": row["score"] or 0}})
        texts = [row["semantic_text"] for row in batch]
        embeddings = generate_embeddings_batch(texts)
        for entry, embedding in zip(entries, embeddings):
            entry["embedding"] = embedding
        for entry in entries:
            store_job_embedding(job_id=entry["job_id"],embedding=entry["embedding"],metadata=entry["metadata"])
        embedded += len(batch)
...
def cleanup_old_jobs():
    ...
    with get_session() as session:
        # Find old job IDs
        unapplied_ids = [jid for (jid,) in session.query(Job.id).filter(
            Job.scraped_at < five_days_ago,
            Job.applied_at == None
        ).all()]

        applied_ids = [jid for (jid,) in session.query(Job.id).filter(
            Job.applied_at < fifteen_days_ago,
            Job.applied_at != None
        ).all()]

        all_old_ids = unapplied_ids + applied_ids

        if all_old_ids:
            # Delete child records first to avoid FK error
            session.query(UserNotification).filter(
                UserNotification.job_id.in_(all_old_ids)
            ).delete(synchronize_session=False)

            session.query(UserApplication).filter(
                UserApplication.job_id.in_(all_old_ids)
            ).delete(synchronize_session=False)

            # Also try to delete embeddings from ChromaDB (best effort)
            try:
                from app.services.vector_store import delete_job_embedding
                for jid in all_old_ids:
                    try:
                        delete_job_embedding(jid)
                    except:
                        pass
            except:
                pass
            ...
            # Now delete jobs
            unapplied = session.query(Job).filter(Job.id.in_(unapplied_ids)).delete(synchronize_session=False) if unapplied_ids else 0
            applied = session.query(Job).filter(Job.id.in_(applied_ids)).delete(synchronize_session=False) if applied_ids else 0
        else:
            unapplied = 0
            applied = 0

        logger.info(f"Cleanup: deleted {unapplied} old jobs + {applied} old applied jobs + {len(all_old_ids)} related notifications/applications.")
...
def get_or_create_user_profile(user_id:int) -> dict:
    with get_session() as session:
        ...
        if not profile:
            profile = UserProfile(user_id=user_id,education = "[]",preferred_keywords = "[]",excluded_keywords = "[]",preferred_locations = "[]")
            session.add(profile)
            session.flush()
...
def _profile_to_dict(profile: UserProfile) -> dict:
    return {
        ...
        "telegram_enabled": profile.telegram_enabled or False,
        "notification_threshold": profile.notification_threshold or 0.6,
        "email_notifications_enabled": profile.email_notifications_enabled if profile.email_notifications_enabled is not None else True,
        "email_digest_enabled": profile.email_digest_enabled if profile.email_digest_enabled is not None else True,

...
function VerifyEmailPage() {
  ...
  return (
    <section className="min-h-screen bg-cream flex items-center justify-center px-6">
      <div className="max-w-md w-full text-center">
        <div className={`w-16 h-16 rounded-full mx-auto mb-6 flex items-center justify-center ${isSuccess ? "bg-emerald-50" : "bg-red-50"}`}>
          {isSuccess ? (
        ...
        <h1 className="font-serif text-3xl text-navy-light mb-3">
          {isSuccess ? "Email Verified" : "Verification Failed"}
        </h1>
        <p className="text-body mb-10 leading-relaxed">
          {isSuccess
            ? "Your email has been verified. You can now sign in and start using AI Job Agent."
            : "This link is invalid or expired. Please sign up again or request a new verification."}
        </p>

        <Link
          to={isSuccess ? "/login" : "/signup"}
          className="inline-flex items-center gap-2 px-8 py-3.5 bg-navy hover:bg-navy-light text-white font-medium rounded-full transition-all shadow-[0_10px_30px_rgba(0,0,0,.08)]">
          {isSuccess ? "Sign In" : "Sign Up"}
          <ArrowRight size={18} />
        </Link>
      </div>
    </section>
